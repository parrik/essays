...
The frontier labs are testing both bets. The big-context bet says transformers will absorb the graph through scale. The MCP/memory-server bet says the graph wants to live outside the context, and the model wants typed adjacency at retrieval time. The argument above is the technical case for the second bet.
