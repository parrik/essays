...
The race to longer context windows — 200K, 1M, 10M, the "infinite-context" pitches — is real progress, and it's also a confession that the substrate hasn't been chosen yet. Self-attention's O(n²) cost is the cost of having no stored structure: every token-pair gets compared because the model doesn't yet know which depends on which. A graph-shaped substrate stores those dependencies once and walks them many times. The context window is a flat list. The limit you keep hitting isn't tokens; it's untyped flatness.
...
- Google ships [million-token contexts](https://blog.google/technology/ai/google-gemini-next-generation-model-february-2024/) and [context caching](https://ai.google.dev/gemini-api/docs/caching) — a different bet, the maximalist version of growing C_n. McCarthy's corollary names this the inefficient path.
...
The frontier labs are testing both bets. The big-context bet says transformers will absorb the graph through scale. The MCP/memory-server bet says the graph wants to live outside the context, and the model wants typed adjacency at retrieval time. The argument above is a technical case for the second bet.
...
**An inverted edge-density prediction.** Pat's Paper 1 Corollary 3 predicts that mature knowledge graphs become edge-dense — more edges per new node over time. True for science, where new findings keep slotting into existing structure. *False for personal psychology*, where new life events spawn new nodes — a person, a loss, a pattern — and cross-time edges stay sparse. A forty-year-old's graph is *node-dense with sparse adjacency*, not edge-dense. Different shape, different design pressures: indexes optimized for "find me the connected clique" don't fit; indexes optimized for "find me the right node, then walk *its* neighborhood" do.

**A tolerance for un-clean action space.** K/A inseparability (Paper 3 Corollary 4) presumes a crisp β overlap structure where knowledge and action share clean boundaries. *"Respond to this friend,"* *"decide this job move,"* *"interpret last night's dream"* don't share one. For personal memory the schema has to tolerate K-without-A and A-without-K coexisting — claims that don't currently route to action, actions not currently grounded in stored claims. The framework's necessity arguments weaken in this regime; the practical engineering has to absorb the slack.

The schema carries over cleanly. The retention logic, as imported from competition-pressure substrates, doesn't survive the move to a self. This essay extends McCarthy's machinery and rewrites the temporal logic for personal memory.
...
The synthesis across all four scales — **inverted index, vector retrieval, typed knowledge graph, AI-native search** — is one shape with the constants reset. Retrieval architectures assumed a human reader; the reader has changed; therefore knowledge representation must change at every scale because the shape is fractal. *That four-scale synthesis is the specific contribution this essay stakes.*

Adjacent prior work names pieces of this without claiming the cross-scale synthesis:

- **Will Bryk** develops the search-specific commercial argument: the [*Why Google Search Sucks for AI*](https://jxnl.co/writing/2025/09/11/why-google-search-sucks-for-ai-will-bryk-exa/) talk works out the AI-native-search slice of the fractal — the differential between human-reader and agent-reader ranking semantics, query format, and result volume.
- **Lù et al. (2025)** [*Build the Web for Agents, Not Agents for the Web*](https://arxiv.org/abs/2506.10953) makes the same-shape claim one level up: the whole web, not just search, was designed for human eyes. The framing aligns; the synthesis still isn't named.
- **Karpathy** has been hinting at this in interviews and his [LLM Wiki gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f), but hasn't claimed it as an essay.
- **Patrick D. McCarthy's** [open-knowledge-graph](https://github.com/patdmc/open-knowledge-graph) develops the necessity theorems formally for Scale 3, with the temporal-logic rewrite this essay names. **Anthropic's** [Claude citations API](https://docs.anthropic.com/en/docs/build-with-claude/citations) ships the same triplet inside the product surface. Older lineage: **RDF** (W3C, 2004), **PROV-O** (W3C, 2013), **Cowan 2001** and **Miller 1956** for bounded capacity.

The synthesis itself — that *the same architectural change is happening at every scale because the shape is fractal* — is what this essay puts down.
...
- **Schema:** typed nodes carrying claim + attribution + derivation — the provenance-triple shape that runs across RDF, scientific provenance, Anthropic's citations API, and McCarthy's open-knowledge-graph (the [know-thyself](https://github.com/parrik/know-thyself) scaffold draws from all of these and adds the temporal-validity axis personal memory needs).
