...
- **Walk:** edge-aware traversal that follows `grounds`, `derives_from`, `emergent_from` so a hit expands into its provenance neighborhood. (Roadmap.)
- **Surface:** an MCP server, so any MCP-aware client can query a personal graph natively. (Roadmap.)
