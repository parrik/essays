...
etudes:
  - label: Three-Path Picker
    url: '#pick-the-path-that-fits'
    note: pick the path for each scenario
  - label: Cascade Simulator
    url: '#how-this-fails-the-cascade'
    note: see the failure mode fire
  - label: Review This PR
    url: '#what-the-failure-costs'
...
(() => {
  ...
  const SCENARIOS = [
    ...
    },
    {
      ...
      best: 'C',
      feedback: {
        A: { verdict: 'wrong', text: 'Centralizing this means a platform team chasing 200 service owners for a change most of them get no value from. The logging campaign becomes a year-long open thread.' },
...
<!---->
  <div class="rp-reveal" hidden>
    ...
    <p class="rp-reveal-body">A human in the loop without context-in-hand has three exits, all bad. The fix isn't "review more carefully" — it's to remove the human from the loop where they can't add value, or give them the context to make the call.</p>
...
(() => {
  ...
  //
  const outcomes = {
    approve: `<p class="rp-out-line"><strong>You approved.</strong></p><p>The PR merges. The build breaks at 3am because the swallowed error masks a config bug. You're now on-call for a change you couldn't read.</p>`,
    changes: `<p class="rp-out-line"><strong>You requested changes.</strong></p><p>@kchen23 doesn't reply for 4 days because the migration isn't their priority. You ping them. They respond <em>"this was generated by RFC-094, not sure who owns it."</em> The PR sits.</p>`,
    skip: `<p class="rp-out-line"><strong>You skipped.</strong></p><p>RFC-094 escalates: <em>"Why hasn't this PR been reviewed?"</em> A new campaign launches: <em>"Track unreviewed migration PRs at PMR."</em></p>`,
    timeout: `<p class="rp-out-line"><strong>Timer ran out — defaulted to skip.</strong></p><p>RFC-094 escalates: <em>"Why hasn't this PR been reviewed?"</em> A new campaign launches: <em>"Track unreviewed migration PRs at PMR."</em></p>`
  };
  //
  let timerId = null;
